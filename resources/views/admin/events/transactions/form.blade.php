<x-app-layout>
    <x-slot name="header">
      <h2 class="text-xl font-semibold leading-tight text-gray-800">
        Event &raquo; {{ $event->name }} &raquo; Transaction &raquo; {{ isset($transaction) ? 'Edit' : 'Buat' }}
      </h2>
    </x-slot>
          ...
          <div class="px-4 py-5 bg-white sm:p-6">
            <form method="POST"
                  action="{{ isset($transaction)
                      ? route('admin.events.transactions.update', [
                          'event' => $event->id,
                          'transaction' => $transaction->id,
                      ])
                      : route('admin.events.transactions.store', $event->id) }}"
                  enctype="multipart/form-data">
              @csrf
              @method(isset($transaction) ? 'PUT' : 'POST')
              <div class="mb-6">
                <label for="name" class="block mb-2 text-sm">Nama Transaction</label>
                <input type="name" name="name" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5"
                       value="{{ isset($transaction) ? $transaction->name : old('name') }}">
              </div>
              <div class="mb-6">
                ...
                <input type="number" name="price" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5"
                       value="{{ isset($transaction) ? $transaction->price : old('price') }}">
              </div>
              <div class="mb-6">
                ...
                <input type="number" name="quantity" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5"
                       value="{{ isset($transaction) ? $transaction->quantity : old('quantity') }}">
              </div>
              <div class="mb-6">
                ...
                <input type="number" name="max_buy" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5"
                       value="{{ isset($transaction) ? $transaction->max_buy : old('max_buy') }}">
              </div>
